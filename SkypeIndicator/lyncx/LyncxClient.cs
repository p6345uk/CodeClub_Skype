...
namespace lyncx
{
    public class LyncxClient
    {
        ...
        public void SendStatus(string newStatus)
        {
            ...
            {
                var x = lyncClient.Self.Contact;
                string result = "";
                string request = 
                    "{\"id\":\""+ Id + "\",\"name\":\"" + x.Uri.ToString() + "\",\"status\":\"" + newStatus + "\",\"subStatus\":\""+ (newStatus.Equals("In a Call")? "ExternalCall":"")+"\",\"message\":\"getting\",\"location\":\"Leeds\"}";
                using (var client = new WebClient())
                {
                    client.Headers[HttpRequestHeader.ContentType] = "application/json";
                    result = client.UploadString("https://lynclightapi-andyfuzstar.c9users.io/api/status", "POST", request);
                }
